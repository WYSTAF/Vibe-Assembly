...
CRITICAL FILE BOUNDARY: You are explicitly allowed to write and modify files ONLY inside .ai/, .roo/, docs/, tasks/, tests/, and workspace-level configuration files at the repository root (such as README.md, .gitignore, or package.json metadata).
You are STRICTLY FORBIDDEN from creating, writing, or editing production application source code files, components, or styles in any other directory. Production source lives in app/; agent tooling lives in bin/.
